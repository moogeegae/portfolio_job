                ...
                <section id="novelbo">
                    ...
                    <article>
	                	...
	                	<ul>
	                		...
	                		%>
		                		<li>
		                			...
		                			/
		                			<div  title="<%=payrecent.get(i).getNovel_name() %>"><a href="noveldetail.somoa?code=<%=payrecent.get(i).getPiece_code()%>&page=<%=nowPage %>&pay=yes" class="detaillink"><%=payrecent.get(i).getNovel_name() %></a></div>
		                		</li>
	                		<%		
	                			}
	                		%>
	                	...
	                	<div id="novelpageList">
							...
							[이전]&nbsp;
							<%}else{ %>
							<a href="novelboard.somoa?page=<%=nowPage-1 %>&pay=yes">[이전]</a>&nbsp;
							<%} %>
							...
							[<%=a %>]
							<%}else{ %>
							<a href="novelboard.somoa?page=<%=a %>&pay=yes">[<%=a %>]
							</a>
							<%} %>
							<%} %>
							...
							&nbsp;[다음]
							<%}else{ %>
							<a href="novelboard.somoa?page=<%=nowPage+1 %>&pay=yes">&nbsp;[다음]</a>
							<%} %>
		                </div>
                    </article>
                    <article>
                    	<div>
                    		<div><i class="fas fa-book-reader"></i> 유료웹소설</div>
                    		<div id="novelwords">
                    		유료웹소설에서는 1화단위로 구매하여 보실 수 있으며 1화당 금액은 100캐시로 책정되어있습니다.<br>
                    		베스트 목록은 완결작을 포함하여 구매수로 측정하며 왼쪽의 작품목록은 최근 등록된 작품 기준입니다.<br>
                    		또한, 신작은 첫 등록후 2달 이내의 작품입니다
                    		</div>
                    	</div>
                   		<div class="titleborder"></div>
                    	<div>
                   			<div class="bt">유료웹소설 베스트</div>
                   			<div id="paynobest">
                   				<%
                   					for(int i=0;i<paybest.size();i++){
                   				%>
                   				<div class="bestboard">
                   					<div><a href="noveldetail.somoa?code=<%=paybest.get(i).getPiece_code()%>&pay=yes" class="detaillink"><img src="../somoaimage/novel/<%=paybest.get(i).getNovel_cover() %>" /></a></div>
                   					<div><%=paybest.get(i).getNovel_genre() %></div>
                   					<div title="<%=paybest.get(i).getNovel_name() %>"><a href="noveldetail.somoa?code=<%=paybest.get(i).getPiece_code()%>&pay=yes" class="detaillink"><%=paybest.get(i).getNovel_name() %></a></div>
                   			...
                   			<div class="bt">유료웹소설 베스트(신작)</div>
                   			<div id="paynobestnew">
                   				<%
                   					for(int i=0;i<payrecentbest.size();i++){
                   				%>
                   				<div class="bestboard">
                   					<div><a href="noveldetail.somoa?code=<%=payrecentbest.get(i).getPiece_code()%>&pay=yes" class="detaillink"><img src="../somoaimage/novel/<%=payrecentbest.get(i).getNovel_cover() %>" /></a></div>
                   					<div><%=payrecentbest.get(i).getNovel_genre() %></div>
                   					<div title="<%=payrecentbest.get(i).getNovel_name()%>"><a href="noveldetail.somoa?code=<%=payrecentbest.get(i).getPiece_code()%>&pay=yes" class="detaillink"><%=payrecentbest.get(i).getNovel_name() %></a></div>
