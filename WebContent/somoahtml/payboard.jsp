    ...
    <body>
        <div id="wrap">
            ...
            <div class="main_site">
                ...
                <%
                	List<Basket> bask = (List<Basket>) request.getAttribute("basket");
                	BookDAO dao = BookDAO.instance();
                	PayDAO pdao = PayDAO.instance();
                %>
                <section id="pay_page">
                    <div>
                        <ul>
                            <li id="payname">장바구니(<%=bask.size() %>개)</li>
                            <li class="topbor"></li>
                            <li>
                                <form action="payment.somoa">
                                	<ul id="payset">
                                		<li>
                                			<div class="pay_info">상품정보</div>
                                			<div class="pay_num">판매가</div>
                                			<div class="pay_num">수량</div>
                                			<div class="pay_num">합계</div>
                                		</li>
                                		<%
                                			for(int i=0;i<bask.size();i++){
                                				String code = bask.get(i).getPiece_code();
							                	Book book = dao.selectOneBook(code);
							                	int price = book.getBook_price();
							                	int inum = pdao.selectBasketNum(result, code);
                                		%>
                                		
                                		<li>
                                			<div class="pay_info" style="display: flex;justify-content: flex-start;">
                                				<div>
                                					<img src="../somoaimage/book/<%=book.getBook_cover() %>" >
                                				</div>
                                				<div style="text-align: left;">
                                					<div style="margin-top:30px;margin-bottom:20px; font-size:20px;"><%=book.getBook_name() %></div>
                                					<div style="margin-bottom:20px;"><%=book.getBook_author() %></div>
                                					<div><%=book.getBook_pub() %></div>
                                				</div>
                                			</div>
                                			<div class="pay_num" style="margin-top: 65px;"><%=price*9/10 %>원</div>
                                			<div class="pay_num" style="margin-top: 65px;"><%=inum %></div>
                                			<div class="pay_num" style="margin-top: 65px;"><%=inum*price*9/10 %>원</div>
            	                    		<input type="hidden" name="code" value="<%=code %>">
        	                        		<input type="hidden" name="inum" value="<%=inum %>">
    	                            		<input type="hidden" name="total" value="<%=inum*price*9/10 %>">
                                	...
                                	<input type="submit" class="action_button" value="결제">
                                </form>
                            </li>    
                            <li class="bottombor"></li>
                        </ul>
                    </div>
                </section>
