...
<html>
    <head>
        <meta charset="utf-8">
        <title><%=article.get(0).getNovel_name() %></title>
                ...
                <section id="novelbo">
                    ...
                    <article>
	                	...
	                	<div id="novelpageList">
							...
							[이전]&nbsp;
							<%}else{ %>
							<a href="noveldetail.somoa?page=<%=nowPage-1 %>&pay=yes&code=<%=article.get(article.size()-1).getPiece_code() %>">[이전]</a>&nbsp;
							<%} %>
							...
							[<%=a %>]
							<%}else{ %>
							<a href="noveldetail.somoa?page=<%=a %>&pay=yes&code=<%=article.get(article.size()-1).getPiece_code() %>">[<%=a %>]
							</a>
							<%} %>
							<%} %>
							...
							&nbsp;[다음]
							<%}else{ %>
							<a href="noveldetail.somoa?page=<%=nowPage+1 %>&pay=yes&code=<%=article.get(article.size()-1).getPiece_code() %>">&nbsp;[다음]</a>
							<%} %>
		                </div>
                    </article>
                    <article id="noveldetailnotice">
                    	...
                    	<%
                    		...
                    		String context = null;
                    		for(int i=0;i<article.size();i++){
                    			if(article.get(i).getNovel_num()==num){
                    				title = article.get(i).getNovel_in_name();
                    				context = article.get(i).getNovel_content();
                    			}
                    		}
                    	%>
                    		<div id="noveldetail_content">
                    			<div><%=title %></div>
                    			<textarea rows="30" cols="70" disabled="disabled" style="resize: none;"><%=context %></textarea>
                    			<div style="display: flex;justify-content: center;">
	                    			<input type="button" onclick="javascript:history.back();" value="목록" style="width: 100px;height: 30px;font-size: 20px;margin-top:20px;" />
                    			</div>
                   			</div>
                    	</div>
                    </article>
