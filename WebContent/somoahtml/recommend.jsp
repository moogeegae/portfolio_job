...
<html>
            ...
            <div class="main_site">
                ...
                <section class="recommend">
                	...
                	<article>
                		...
                		<div id="recombtn">
                			<input type="button" onclick="search()" value="장르별 검색">
                			<%
                				if(result==null){
                			%>
                			<a href="loginsession.jsp?lgrt=recommend"><button>글쓰기</button></a>
                			<%		
                				}else{
                			%>
                			<input type="button" onclick="wgo()" value="글쓰기">
                			<%
                				}
                			%>
                			<input type="button" onclick="likego()" value="추천순으로 보기" style="width:135px; ">
                		</div>
                	</article>
                    <article><!-- 책 목록 -->
                    		<%
                    			String genrename=null;
                    			List<Recommend> list = (List<Recommend>) request.getAttribute("list");
                    			List<Recommend> listgenre = (List<Recommend>) request.getAttribute("listgenre");
                    			genrename = (String) request.getAttribute("GenreName");
                    			int reno = 0;
                    			String genre = null;
                    			String cover = null;
                    			String title = null;
                    			...
                    			if(genrename==null){
                    				for(int i = 0;i<list.size();i++){
	                    				reno = list.get(i).getReno();
                    					genre = list.get(i).getGenre();
	                    				cover = list.get(i).getCover();
	                    				title = list.get(i).getTitle();
	                    				recommender = list.get(i).getRecommender();
	                    				date = list.get(i).getDate();
	                    				count = list.get(i).getCount();
	                    				up = list.get(i).getUp();
	                    				content = list.get(i).getContents();
	                    				code = list.get(i).getCode();
	                    		%>
		                    	<div class="listrecom">
		                    		<div><img src="../somoaimage/novel/<%=cover %>" /></div>
		                    		<div class="listindex">
		                    			<div><a href="RecomDetail.somoa?reno=<%=reno %>"><%=title %></a></div>
		                    			<div><%=content %></div>
		                    			<div class="listinfo">
		                    			...
		                    			<div class="listbookinfo">
		                    				<span><%=genre %></span>
		                    				<span><%=title %></span>
		                    				<span>작가</span>
		                    				<span>도서 코드: <%=code %></span>
		                    			</div>
		                    		</div>
		                    	</div>
	                        	<%
		                    	...
		                    	<%		
	                    				...
	                    				for(int j=0;j<listgenre.size();j++){
	                    					reno = listgenre.get(j).getReno();
	                    					genre = listgenre.get(j).getGenre();
		                    				cover = listgenre.get(j).getCover();
		                    				title = listgenre.get(j).getTitle();
		                    				recommender = listgenre.get(j).getRecommender();
		                    				date = listgenre.get(j).getDate();
		                    				count = listgenre.get(j).getCount();
		                    				up = listgenre.get(j).getUp();
		                    				content = listgenre.get(j).getContents();
		                    				code = listgenre.get(j).getCode();
		                    	%>
			                    <div class="listrecom">
		                    		<div><img src="../somoaimage/novel/<%=cover %>" /></div>
		                    		<div class="listindex">
    ...
    <script>
		...
		function wgo(){
			$('#formselect').attr('action','MyRecomWrite.jsp').submit();
		};
		function likego(){
			location.href = 'RecomLike.somoa';
		}
